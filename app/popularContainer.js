...
const PopularContainer = ({ children }) => {
    ...
    const allTypes = ['Entries','Courses','Tools','Profiles'];
    ...
    return (
        <div className='bg-gray text-tan w-full pb-24'>
            ...
            <div className='mx-auto mt-6 flex justify-center items-center max-w-max border-2 bg-darkGray border-tan text-center rounded-full first:-ml-[2px] first:rounded-l-full last:-mr-[2px] last:rounded-r-full'>
                {allTypes.map((type, i) => (
                    <div className={`px-4 py-1 cursor-pointer hover:bg-tan hover:text-gray duration-150 ${i === 0 && '-ml-[2px] rounded-l-full'} ${i === 3 && '-mr-[2px] rounded-r-full'}
